...
void create_monkey(Monkey* new_monkey, int items[8], int operation_type, int operation_factor, int test, Monkey* true, Monkey* false){
    ...
    for (int i = 0; i < 8; i++){
        if (items[i] != 0){
            ...
            new_monkey->items[new_monkey->current_item_index] = items[i];
        }
    }
...
int main () {
    ...
    printf("Finished %d", high*sec_high);
    return 0;
}